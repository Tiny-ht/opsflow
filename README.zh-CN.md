...
- `agent docs`：生成 `AGENTS.md` 和 `CLAUDE.md`，让 AI 代理知道什么时候、怎样自主记录工作
...
`AGENTS.md` 和 `CLAUDE.md` 会生成在项目根目录。它们会提醒 AI 代理读取 Opsflow 配置、检查当前状态和 handoff、追加 trace/eval 记录，并避免改写历史。已有的代理说明文件不会被覆盖，除非传入 `--force-docs`。
